...
## Chapter 8 - Integrating inference and intelligence
...
:warning: **WARNING:** Unlike the other chapters in the book that all use Python **3.13**, I needed to use Python **3.11** with AutoGluon in Chapter 8. Please install Python 3.11 and use that to create the virtual environment for the Chapter 9 code samples. Instructions on how to do this are provided below.
...
#### 8.1 Resample PowerPal electricity consumption data to hourly
...
`
python ch08_code02_biegel.py
`

#### 8.3 Calculate lagged features for indoor temperature
This example demonstrates calculating 1,2, and 3 hour lagged features for indoor temperature data.

Run it with the following command

`
python ch08_code03_biegel.py
`

#### 8.4 Calculate Z-score for indoor air quality of my home for November 2025
...
#### 8.5 Create an isolation forest ML model to detect anomalies in power and voltage consumption by my refrigerator
...
#### 8.6 Create a histogram gradient boosting regressor ML model to predict indoor temperatures
...
#### 8.7 Create an AutoML Timseries predictor with AutoGluon to predict net energy usage in my home
...
`
python ch08_code07_biegel.py
`

To create a visualization of its prediction over 48 hours, versus actual values, run the following command

`
python ch08_code07_biegel_visualize.py
`

#### 8.8 Create an image classifier to determine if my garage is open or shut with AutoGluon's MultiModal predictor
...
`
python ch08_code08_biegel_train.py
`

To run a prediction with the model, run the following command

`
python ch08_code08_biegel_predict.py
`

#### 8.9 Perform retrieval augmented generation using a vector store of embeddings of my appliance manuals
...
#### 8.10 A Strands SDK tool example
This example creates a tool using the Strands SDK that a Strands agent can use to get solar PV data from my Fronius Symo inverter. It cannot be run alone, but is used by the agent in 8.11.

#### 8.11 A Strands agent example
