...
if MEMGRAPH_USER and MEMGRAPH_PASSWORD:
    # With authentication: bolt://username:password@host:port
    MEMGRAPH_URI = (
        f"bolt://{MEMGRAPH_USER}:{MEMGRAPH_PASSWORD}@{MEMGRAPH_HOST}:{MEMGRAPH_PORT}"
    )
elif MEMGRAPH_HOST:
    # Without authentication: bolt://host:port
...
try:
    driver = GraphDatabase.driver(MEMGRAPH_URI, auth=basic_auth("user", "pass"))
