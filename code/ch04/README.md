...
#### 4.4 Example of using SQL to directly query data in a Parquet file using DuckDB
...
#### 4.5 Example of using delta tables to enable ACID transactions over Parquet files
...
#### 4.6 Example of batch ingestion to a SQL database followed by querying the data with SQL
...
#### 4.7 Example of ingesting a stream of events related to the UK rail network and running a sliding window over them
You will need to register for an account [here](https://publicdatafeeds.networkrail.co.uk/) in order to run this example. Its a simple signup process. How late are the trains running today?
...
Here is an example of the output, including calculation of percentage of on-time movements in a sliding window

![TrainMovements](images/train_movements.gif)

#### 4.8 An example of querying a NASA REST API to get an image of the largest man made lake on the planet
...
#### 4.9 Using Apache Spark to summarize power consumption data
