...
## Chapter 9 - Predicting outcomes with simulation
...
To create a virtual environment named *dtia_ch09*, type the following command:

`python3 -m venv dtia_ch09`

You can then activate the virtual environment by typing the following command

`source dtia_ch09/bin/activate`

The libraries that are required to run the chapter 9 code samples are defined in the requirements.txt file in this directory. To install the required libraries in your virtual environment, run the following command
...
#### 9.1 Solve an ordinary differential equation with Python
...
#### 9.3 Run a simulation using an FMU created from the OpenModelica model
...
#### 9.4 Integrating sensor data with continuous simulation
...
#### 9.6 Discrete event simulation of home power consumption
...
`
python ch09_code06_biegel_montecarlo.py
`

#### 9.7 Example of FEM using FreeFEM
...
docker run --rm \
    ...
    -nw /data/ch09_code07_biegel.edp    
```


#### 9.8 CFD example using JAX to simulate the movement of cool air from an air conditioner, through a room
...
#### 9.9 A simple linear regression based reduced order model (ROM)
